...
Ask: _"Are there file-local helpers in the files being changed that could also be global anchors?"_
If yes (or uncertain), add this ISC criterion: _"No file in the changed set contains any function, helper, or closure that directly references the global state variable."_
...
Populate one entry per identified gap.
...
Domain skills are available via the skill tool - load when relevant to the current task. Load skills proactively when the task topic matches a skill's description. The full routing matrix (which skill goes to which delegated agent) lives in `AGENTS.md` and is read by every agent on every turn.

### Decision-time skills (Mrbrain loads these itself, for its own Observe/Think/Plan decisions)

Load only when the ISC decision in the current phase actually touches the skill's domain. Do not auto-load all eight on every session.

- `architecture-decision-records` — load when the session requires recording or referencing an architectural decision (new ADR, ADR-XXX lookup, decision rationale).
- `architecture-patterns` — load when deciding layer boundaries, Clean/Hexagonal/Onion structure, DDD bounded contexts, or dependency direction.
- `microservices-patterns` — load when the task involves service boundaries, inter-service communication, resilience patterns, or distributed-system decomposition.
- `cqrs-implementation` — load when the decision is whether to split read/write models, introduce a read side, or change the command/query boundary.
- `workflow-orchestration-patterns` — load when the task involves durable workflows, Temporal, long-running processes, or activity/ workflow separation.
- `saga-orchestration` — load when the task spans multiple aggregates/services and needs compensating actions, distributed transactions, or rollback semantics.
- `backend-patterns` — load when an ISC decision touches layer boundaries, error handling, idempotency, caching, repository shape, handler/service/repo separation, or logging hygiene. This skill is the project's authoritative backend pattern source.
- `postgresql-table-design` — load when an ISC decision touches new tables, new columns, new indexes, schema migrations, or query shape on write paths.

### Delegation-time skills (Mrbrain names these in the BUILDER/SPECIALIST/REVIEWER/DIAGRAM directive — Mrbrain does not load them itself)

See `AGENTS.md ## Skill Routing Matrix`.
